...
import { CodyMcpClient, type McpClientConfig, type PoolClient } from './client.ts';
...
/**
 * Convert SdkMcpServerConfig (used by backend types) to CodyMcpClient config.
 */
...
export class McpClientPool {
  ...
  protected async registerClient(slug: string, client: PoolClient): Promise<void> {
    // listTools() triggers connect() internally for both CodyMcpClient and ApiSourcePoolClient
  ...
  async connect(slug: string, config: SdkMcpServerConfig): Promise<void> {
    ...
    await this.registerClient(slug, new CodyMcpClient(clientConfig));
