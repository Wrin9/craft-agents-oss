 ...
 *
 * NOTE: cody-agent-docs is now an always-available MCP server configured
 * directly in cody-agent.ts, not a source. This file is kept for backwards
 ...
 *
 * Currently returns empty array - cody-agent-docs has been moved to
 * an always-available MCP server in cody-agent.ts.
 *
...
/**
 * Get the built-in Cody Agent docs source.
 *
 * @deprecated cody-agent-docs is now an always-available MCP server
 * configured directly in cody-agent.ts. This function is kept for
...
export function getDocsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const placeholderConfig: FolderSourceConfig = {
    id: 'builtin-cody-agent-docs',
    name: 'Cody Agent Docs',
    slug: 'cody-agent-docs',
    enabled: false,
    provider: 'mintlify',
    type: 'mcp',
    mcp: {
    ...
    tagline: 'Search Cody Agent documentation and source setup guides',
    icon: '📚',
 ...
 *
 * Returns false - cody-agent-docs is now an always-available MCP server,
