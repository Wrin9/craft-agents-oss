 ...
 *
 * Stores credentials in an encrypted file at ~/.cody-agent/credentials.enc
...
// File location
const CREDENTIALS_DIR = process.env.CODY_CONFIG_DIR || process.env.CODY_CONFIG_DIR || join(homedir(), '.cody-agent');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('cody-agent-v2') // Bumped version for new key derivation
      .digest();
