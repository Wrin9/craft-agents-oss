 ...
 *
 * Loads release notes from bundled assets and syncs them to ~/.cody-agent/release-notes/.
...
const CONFIG_DIR = process.env.CODY_CONFIG_DIR || process.env.CODY_CONFIG_DIR || join(homedir(), '.cody-agent');
...
function loadBundledReleaseNotes(): Record<string, string> {
  ...
  // Try bundled assets first, fall back to ~/.cody-agent/release-notes/
