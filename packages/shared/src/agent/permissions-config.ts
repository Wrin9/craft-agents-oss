 ...
 *
 * File locations:
 * - Workspace: ~/.cody-agent/workspaces/{slug}/permissions.json
 * - Per-source: ~/.cody-agent/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.cody-agent/permissions/
 * Reads env var dynamically so tests can override via CODY_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.CODY_CONFIG_DIR || process.env.CODY_CONFIG_DIR || join(homedir(), '.cody-agent');
...
/**
 * Load default permissions from ~/.cody-agent/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.craftAgentsCli && pattern.startsWith('^cody-agent\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.cody-agent/permissions/default.json)
   ...
   * These come from ~/.cody-agent/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping cody-agent bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
