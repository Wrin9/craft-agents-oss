...
// ============================================================
// Cody Agent Docs Upstream Proxy
// ============================================================
...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`Cody Agent Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Cody Agent Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Cody Agent Docs server is not connected. Tool '${name}' unavailable.`);
  }
