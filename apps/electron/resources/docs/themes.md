...
This guide explains how to customize the visual theme of Cody Agent.

## Overview

Cody Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.cody-agent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.cody-agent/theme.json` - Override specific colors (app-level)
...
```
~/.cody-agent/workspaces/{id}/config.json
```
...
Create `~/.cody-agent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.cody-agent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.cody-agent/themes/{name}.json`
...
1. Create `~/.cody-agent/theme.json` for overrides or `~/.cody-agent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.cody-agent/theme.json` for overrides, `~/.cody-agent/themes/` for presets)
