...
This guide explains how to create and configure skills in Cody Agent.

> **CLI-first workflow (recommended):** Use `cody-agent skill ...` commands instead of editing `SKILL.md` files directly.
> - `cody-agent skill --help`
...
Cody Agent uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in Cody Agent
...
1. **Workspace skill checked first** - If `~/.cody-agent/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.cody-agent/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.cody-agent/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.cody-agent/workspaces/{ws}/skills/commit/SKILL.md`
