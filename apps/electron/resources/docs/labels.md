...
> **CLI-first workflow (recommended):** Use `cody-agent label ...` commands instead of editing JSON directly.
> - `cody-agent label --help`
> - Canonical command reference: [cody-cli.md](./cody-cli.md)

## Storage Locations

- Config: `~/.cody-agent/workspaces/{id}/labels/config.json`
...
Prefer `cody-agent` commands:

```bash
cody-agent label create --name "Bug" --color "destructive"
cody-agent label create --name "Priority" --color "accent" --value-type number
cody-agent label create --name "Due Date" --color "info" --value-type date
cody-agent label create --name "Project" --color "foreground/60"
cody-agent label create --name "Alpha" --color "info" --parent-id project
cody-agent label create --name "Beta" --color "success" --parent-id project
```
