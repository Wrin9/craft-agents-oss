...
When your OAuth token expires, Cody Agent automatically refreshes it in the background using the refresh token. No manual re-authentication required — your MCP connections stay alive.
...
Configure sources that require multiple authentication headers (e.g., API key + tenant ID). See the updated documentation at `~/.cody-agent/docs/sources.md`.
